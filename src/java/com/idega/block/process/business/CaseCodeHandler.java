/*
 * $Id: CaseCodeHandler.java,v 1.2 2007/05/21 09:57:34 valdas Exp $
 ...
 */
public class CaseCodeHandler implements ICPropertyHandler {
	...
	public PresentationObject getHandlerObject(String name, String stringValue, IWContext iwc, boolean oldGenerationHandler) {
		...
		try {
			Collection codes = getCaseBusiness(iwc).getCaseCodes();
			Iterator iter = codes.iterator();
			while (iter.hasNext()) {
				CaseCode caseCode = (CaseCode) iter.next();
				menu.addMenuElement(caseCode.getCode(), caseCode.getDescription() != null ? caseCode.getDescription() : caseCode.getCode());
			}
		}
